...
authenticated_actor if actor_id != ""

has_role(r) if roles[_] == r

write_role_ok if has_role("admin")
write_role_ok if has_role("operator")

# Deny if no roles (write only; reads handled separately).
deny_write if write_action and not write_role_ok
